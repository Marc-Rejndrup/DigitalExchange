...
public class RevenueBySymbolServlet {
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			java.sql.ResultSet rs = stmt1.executeQuery(
					  "SELECT O.Symbol, SUM(O.CompPrice * O.NumShares) AS Revenue"
					+ "FROM Orders O"
					+ "WHERE O.OrderType = 'S'"
					+ "GROUP BY Symbol"
			);
			...
			request.setAttribute("RevenueSymbolTable", list);
			rs.close();
			conn.close();
		} catch(Exception e){
			e.printStackTrace();
		...
		RequestDispatcher view = request.getRequestDispatcher("revenueSymbol.jsp");
